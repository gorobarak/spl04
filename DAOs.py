class _Vaccines:
    ...
    def sendVaccines(self, amount):
        ...
        while amount > 0:
            ...
            id = oldestVac[0][0]
            availableQuantity = oldestVac[0][1]
...
class _Suppliers:
    ...
    def getId(self, supplierName):
        c = self._conn.cursor()
        return c.execute("SELECT id FROM suppliers WHERE name = ?", supplierName).fetchone()

    def getLogistic(self, supplierId):
        c = self._conn.cursor()
        return c.execute("SELECT logistic FROM suppliers WHERE id = ?", supplierId).fetchone()
...
class _Clinics:
    ...
    def getId(self, location):
        c = self._conn.cursor()
        return c.exexute("SELECT id FROM clinincs WHERE name = ?", location).fetchone()

    def getLogisticId(self, clinicId):
        c = self._conn.cursor()
        return c.exexute("SELECT logistic FROM clinics WHERE id = ?", clinicId).fetchone()
